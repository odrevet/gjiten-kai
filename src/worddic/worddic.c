...
void worddic_init(worddic *p_worddic)
{
  ...
  if (err != NULL)
  {
    g_printerr("Error while loading worddic definitions file: %s\n",
               err->message);
    g_error_free(err);
    gtk_main_quit();
  }
...
void print_entries(GtkTextBuffer *textbuffer, worddic *p_worddic)
{
         ...
         entry_number <= p_worddic->entries_per_page)
  {
    ...
    //comment
    if (comment)
      print_unit(textbuffer, comment, &p_worddic->conf->notes);
    ...
    for (unit = entry->sense; unit != NULL; unit = unit->next)
    {
           ...
           gloss_list = gloss_list->next)
      {
        ...
        if (lang_activated)
          gloss_to_print = g_slist_prepend(gloss_to_print, p_gloss);
      }
