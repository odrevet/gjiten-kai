...
extern G_MODULE_EXPORT void on_gjitenkai_menuitem_history_click(GtkWidget *menuitem_history, gjitenkai *gjitenkai);
...
void gjitenkai_init(gjitenkai *gjitenkai)
{
  ...
  if (err != NULL)
  {
    g_printerr("Error while loading gjitenkai definitions file: %s\n",
               err->message);
    g_error_free(err);
    gtk_main_quit();
  }
